...
execute as @a[scores={Voidblock_multiplayer_reset=1}] run function voidblock:versions/pid_setup


##Voidblock PID VERSION WARNING (CAMBIARE SOLO SE NON CI SONO CONVERSIONI, DISABILITARE LA SECONDA RIGA SE C'E IL PID)
#execute as @a[scores={1.21_PID_0=30}] run tellraw @s {"translate":"Voidblock.versions_id.warning_update.on","italic":true,"color":"#d5d5d5"}
execute as @a[scores={1.21_PID_0=30}] run tellraw @s {"translate":"Voidblock.versions_id.warning_update.off","italic":true,"color":"#d5d5d5"}
...
execute as @a[scores={PID_lts_warning=15}] run tellraw @a [{"translate":"Voidblock.setup_version.lts_warning","bold":true,"color":"red"},{"text":" "},{"text":"??? ","bold":true,"color":"red"},{"translate":"Voidblock.setup_version.lts_warning.month","bold":true,"color":"red"}]
...
execute as @a[scores={PID_warning=5}] run tellraw @a ["",{"translate":"Voidblock.setup_version.warning","bold":true,"color":"red"},{"translate":"Voidblock.setup_version.warning.download","bold":true,"italic":true,"underlined":true,"color":"red","clickEvent":{"action":"open_url","value":"https://github.com/Loweredgames/Voidblock/releases"}}]
