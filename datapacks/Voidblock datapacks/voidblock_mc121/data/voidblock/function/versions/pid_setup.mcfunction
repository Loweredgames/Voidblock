...
scoreboard players add @a Voidblock_pid_1_mc121 1


##SETUP VERSION Voidblock_ID (CAMBIARE LA SCORE IN "Voidblock_id". VEDERE LA SCORE PID_X PER RIFERIMENTO)
execute as @a[scores={Voidblock_id=30}] run scoreboard players set @a Voidblock_pid_1_mc121 -100


##VERSION IMPORT LOCK (BLOCCA LA CONVERSIONE SE SI USA IMPORT)
execute as @a[scores={Voidblock_import=1..100}] run scoreboard players set @a Voidblock_pid_1_mc121 2400