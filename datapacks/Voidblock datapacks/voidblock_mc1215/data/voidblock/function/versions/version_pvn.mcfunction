...
scoreboard objectives add 1.21.5_PVN_0d dummy
scoreboard players add @a 1.21.5_PVN_0d 1
execute as @a[scores={Voidblock_id=10}] run scoreboard objectives remove 1.21.5_PVN_0d
execute as @a[scores={Voidblock_import=20}] run scoreboard objectives remove 1.21.5_PVN_0d
...
#execute as @a[scores={1.21.5_PVN_0d=6}] run tellraw @s ["",{"text":"\u26a0WARNING: You are using a version of Minecraft that is still in development or untested, bugs may occur and the world may become corrupted and damaged. always make backups. Wait for it to be fully released.\u26a0","color":"red"}]


##PVN VERSION PROTOCOL LOOP (NON TOCCARE MAI)
execute as @a[scores={1.21.5_PVN_0d=900000000..}] run scoreboard players set @a 1.21.5_PVN_0d 0